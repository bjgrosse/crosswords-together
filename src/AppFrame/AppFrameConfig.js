...
const AppFrameConfig = props => {
  ...
  useEffect(() => {
    ...
    if (store.appBar && history.action === "PUSH") {
      window.history.replaceState(
        { ...window.history.state, appBarCanGoBack: true },
        null
      );
    }
    store.setAppBar({
      ...
      showBackButton:
        props.showBackButton !== undefined
  ...
  return (
    <>
      {context.appBarContentNode &&
        content &&
        ReactDOM.createPortal(content, context.appBarContentNode)}
      {context.appBarActionsNode &&
        ReactDOM.createPortal(props.appBarActions, context.appBarActionsNode)}
      {context.bannerNode &&
        appBanners &&
        ReactDOM.createPortal(appBanners, context.bannerNode)}
      {props.children}
    </>
  );
};
