...
const ClueLists = observer(props => {
  ...
  const ref = useRef();
  const showTabs =
    useMediaQuery("(max-width:960px) and (orientation: landscape") ||
    width < 400;
  ...
  if (showTabs) {
    content = (
      <>
        ...
        >
          {wordLists.map(list => (
            <TabItem key={list.title} pb={0.5} label={list.title} />
          ))}
        </TabList>
        <Divider />
        <ClueList
    ...
    content = wordLists.map((list, index) => (
      <Div key={list.title} flex alignStretch grow>
        <ClueList
          showTitle={true}
          title={list.title}
          words={list.words}
