...
class Page extends React.Component {
  ...
  setFocusedSquareRef = element => {
    ...
    if (
      !this.state.focusedSquareElement ||
      this.state.selectedWord !== this.state.keyboardSelectedWord
    ) {
      this.setState({
        focusedSquareElement: element,
        keyboardSelectedWord: this.state.selectedWord
      });
    } else {
  ...
  setFocusedSquareRefDelayed = Debounce(
    element =>
      this.setState({
        focusedSquareElement: element,
        keyboardSelectedWord: this.state.selectedWord
  ...
  handleKeyDown = safeHandlerWarn(this, function(e) {
    ...
    if (e.key.length === 1 && e.key.match("[a-zA-Z0-9]")) {
      ...
      switch (e.key) {
        ...
        case "Backspace":
          if (
            focusedCell.value &&
            (focusedCell.selectedDirectly || focusedCell.valueJustSet)
          ) {
            clearValue = true;
          } else {
            move--;
    ...
    if (move !== 0) {
      if (direction === "h") {
        if (
          x + move >= 0 &&
          x + move < puzzle.squares[y].length &&
          !puzzle.squares[y][x + move].isBlocked
        ) {
          newX = x + move;
        }
      } else {
        if (
          y + move >= 0 &&
          y + move < puzzle.squares.length &&
          !puzzle.squares[y + move][x].isBlocked
        ) {
          newY = y + move;
        }
      }
  ...
  render() {
    const { puzzle } = this.props;
    const noticePopover =
      this.props.noticePopover && this.props.noticePopover();
    const puzzleBoard = <PuzzleBoard puzzle={this.props.puzzle} />;
    const keyboardFab = (
      <KeyboardFab
        show={!this.state.isKeyboardOpen && puzzle.selectedWord}
        handleClick={this.handleShowKeyboard}
      />
    );
