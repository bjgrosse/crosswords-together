...
import is, { isOr } from "Utility/styledIs";
...
//   CSSColors[Math.floor(Math.random() * CSSColors.length - 1)];
...
export const Paper = styled(Div).attrs(p => ({
  bgcolor: p.bgcolor || p.theme.palette.background.paper
}))`
  border-radius: 5px;
  box-shadow: 0px ${p => (p.elevation || 1) * -1}px
      ${p => (p.elevation || 1) * 3}px 0px rgba(0, 0, 0, 0.2),
    1px ${p => (p.elevation || 1) * 1}px ${p => (p.elevation || 1) * 3}px 0px
      rgba(0, 0, 0, 0.2);
`;
export const SubTitle1 = styled(Typography).attrs(props => ({
  variant: "h6",
  ml: props.ml !== undefined ? props.ml : 1,
  mb: props.mb !== undefined ? props.mb : 0.5,
...
export const SubTitle2 = styled(SubTitle1).attrs(props => ({
  variant: "button",
